...
class Edit extends \Magento\Backend\App\Action
{
    ...
    protected function _initAction()
    {
        ...
        $resultPage->setActiveMenu('Dynamic_DynamicVideos::Dynamicvideos')
            ->addBreadcrumb(__('Dynamic DynamicVideos'), __('Dynamic DynamicVideos'))
    ...
    public function execute()
    {
        ...
        $this->_coreRegistry->register('dynamicvideos', $model);

        // 5. Build edit form
        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->setActiveMenu('Dynamic_DynamicVideos::dynamicvideos');
        $resultPage->addBreadcrumb(__('Dynamic'), __('Dynamic'));
        $resultPage->addBreadcrumb(
            $id ? __('Edit Video') : __('New Video'),
            $id ? __('Edit Video') : __('New Video')
        );
        $resultPage->getConfig()->getTitle()->prepend($id ? __('Edit Video') : __('New Video'));
